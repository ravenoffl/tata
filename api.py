...
def configure_ngrok():
    ngrok_token = "2m1XhcURUEdT3DLvTAgJ3mMiTGn_6gD8NJPmMHSof81LAzHVA"
    try:
...
def update_soul_txt(public_url):
    with open("aman1.txt", "w") as file:
        file.write(public_url)
    print(f"New ngrok link saved in aman1.txt")
...
def update_vps_soul_txt(public_url):
    ...
    try:
        ...
        sftp = ssh.open_sftp()
        with sftp.open("aman1.txt", "w") as file:
            file.write(public_url)
        sftp.close()
        ssh.close()
        print("Updated aman1.txt on VPS successfully.")
    except Exception as e:
        print(f"Failed to update aman1.txt on VPS: {str(e)}")
